...
function createTargets(target_size, horizontal_gap, vertical_gap)
{
  ...
  h_margin = horizontal_gap / (GRID_COLUMNS + 7.5);
...
*/
  ...
  bloco_x = 380 + bloco_col * (bloco_largura);
  bloco_y = 40 + bloco_row * (bloco_altura - 10);
...
function windowResized() 
{
  if (fullscreen())
  {
    ...
    let horizontal_gap = screen_width - (target_size+0.8) * GRID_COLUMNS;// empty space in cm across the x-axis (based on 10 targets per row)
    let vertical_gap   = screen_height - (target_size+0.8) * GRID_ROWS;  // empty space in cm across the y-axis (based on 8 targets per column)
